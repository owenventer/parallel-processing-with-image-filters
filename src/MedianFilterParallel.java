  ...
  protected void computeDirectly() {
    ...
    protected void compute() {
      if(arrSize<threshold){
        ...
        int split=arrSize/2;
        MedianFilterParallel left=new MedianFilterParallel(split,startY, startX, endY, (endX-split)+((frameSize-1)/2));
        MedianFilterParallel right= new MedianFilterParallel(arrSize-split, startY, (startX+split)-((frameSize-1)/2), endY, endX);
        left.fork();
        right.compute();
        left.join();
